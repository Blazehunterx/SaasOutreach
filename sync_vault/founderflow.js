...
app.post('/api/capture-session', async (req, res) => {
    ...
    if (loginBrowser) {
        // 🏁 v1.29 TITANIUM SEAL: Extract Hot-Reload State before closing
        ...
        console.log(`✅ TITANIUM SEAL COMPLETED: ${settings.platform.toUpperCase()}`);
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        console.log(`📡 Opening Titanium Login Portal: ${settings.platform.toUpperCase()}`);
...
async function startOutreach(socket) {
    ...
    try {
        ...
        socket.emit('log', { type: 'info', message: `🚀 Starting TITANIUM Outreach [${settings.platform.toUpperCase()}] v1.58...` });
        
        if (!leads || leads.length === 0) {
            socket.emit('log', { type: 'warn', message: '⚠️ Lead list is empty. Add leads first.' });
        ...
        if (!isGmail) {
            socket.emit('log', { type: 'info', message: `✅ Session Validated. Launching Titanium v1.58 hardware...` });
            
            const profileDir = path.resolve(SESSION_DIR, 'profiles', settings.platform);
            if (!fs.existsSync(profileDir)) fs.mkdirSync(profileDir, { recursive: true });

            // 🏁 v1.29 GHOST SHELL: PERSISTENT HARDWARE
            ...
            // 🏁 v1.29 Titanium Inject: Re-inject Hot Backup
            if (fs.existsSync(sessionFile)) {
                const storage = JSON.parse(fs.readFileSync(sessionFile, 'utf8'));
                if (storage.cookies) {
                    await browser.addCookies(storage.cookies);
                    socket.emit('log', { type: 'info', message: `🔐 Titanium State Injected: ${storage.cookies.length} sessions re-hydrated.` });
                }
            }
            
            page = browser.pages()[0] || await browser.newPage();
        } else {
            socket.emit('log', { type: 'info', message: `✅ SMTP Nexus Validated. Launching Browserless Outreach...` });
        }
        ...
        for (const lead of leads) {
            ...
            // 🛑 1. Session Limit (Dynamic)
            if (processedLeads >= (settings.sessionLimit || 160)) {
                socket.emit('log', { type: 'warn', message: `🏁 Session Limit (${settings.sessionLimit}) reached. Shutting Down.` });
                break;
            }

            // 🛑 2. Batch Pause (Dynamic)
            if (batchCount >= (settings.batchSize || 20)) {
                const elapsed = Date.now() - batchStartTime;
                if (elapsed < 60 * 60 * 1000) {
                    const waitTime = (60 * 60 * 1000) - elapsed;
                    socket.emit('log', { type: 'info', message: `⏸️ Ghost Protocol: ${settings.batchSize} leads sent. Resting for ${Math.round(waitTime/60000)}m...` });
            ...
            try {
                if (settings.platform === 'ig') {
                    ...
                    if (result === 'FATAL') {
                        socket.emit('log', { type: 'warn', message: '🛑 FATAL FAULT: Stopping Outreach to preserve browser state.' });
                        outreachActive = false;
                        break;
                    }
                ...
                // ⏳ 3. Dynamic Humanized Delay
                const min = (settings.minDelay || 120) * 1000;
                const max = (settings.maxDelay || 240) * 1000;
                const delay = Math.floor(Math.random() * (max - min)) + min;
                socket.emit('log', { type: 'info', message: `🧬 Ghost Protocol: Humanizing for ${Math.round(delay/1000)}s...` });
                if (page) await page.waitForTimeout(delay);
                else await new Promise(r => setTimeout(r, delay));
            } catch (driverErr) {
                socket.emit('log', { type: 'error', message: `❌ Driver Error on ${lead.url.substring(0, 20)}: ${driverErr.message}` });
                socket.emit('progress', { url: lead.url, status: '❌ Failed' });
            }
        }
    } catch (e) {
        socket.emit('log', { type: 'error', message: `❌ HARDWARE CRASH: ${e.message}` });
    } finally {
        outreachActive = false;
        if (browser) await browser.close();
        socket.emit('log', { type: 'info', message: '🏁 Outreach Session Ended.' });
    }
}
...
server.listen(3000, () => {
    console.log('🚀 Founderflow UNIVERSAL Hub LIVE at http://localhost:3000');
});
