  ...
  return (
    <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        {(status === 'APPROVED' || status === 'BAN') && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#111] border border-white/10 rounded-3xl p-12 shadow-3xl text-center">
            <div className={`w-20 h-20 ${status === 'APPROVED' ? 'bg-green-500/10' : 'bg-red-500/10'} rounded-full flex items-center justify-center mx-auto mb-8`}>
              <ShieldCheck size={40} className={status === 'APPROVED' ? 'text-green-500' : 'text-red-500'} />
            </div>
            <h2 className="text-2xl font-bold text-white mb-4 tracking-tighter">
              {status === 'APPROVED' ? 'Trial Fully Activated' : 'Access Restricted'}
            </h2>
            <p className="text-white/40 text-sm leading-relaxed mb-10">
              {status === 'APPROVED' ? 'Access Granted. Your 7-day Premium Trial is now live.' : 'Your identity could not be verified by the elite sentinel.'}
            </p>
            {status === 'APPROVED' && (
              <div className="space-y-4">
                <a 
                  href="/Founderflow_Launcher.bat" download
                  className="w-full bg-blue-500 text-white font-bold py-5 rounded-2xl flex items-center justify-center gap-3 hover:bg-blue-600 transition-all shadow-lg"
                >
                  Download Connection Hub
                  <Settings size={20} />
                </a>
                <button 
                  className="w-full bg-white/5 border border-white/10 text-white/60 font-bold py-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-white/10 transition-all"
