...
export default function DashboardPage() {
  ...
  const fetchProfile = async () => {
    ...
    // 🛡️ SOVEREIGN BYPASS: The founder never lands on onboarding
    const isMarvin = user.email && user.email.includes('marvin');
    
    if (data?.status !== 'TRIAL' && data?.status !== 'PERMANENT' && !data?.is_admin && !isMarvin) {
